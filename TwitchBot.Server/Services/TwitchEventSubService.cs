...
namespace TwitchBot.Server.Services
{
    ...
    public class TwitchEventSubService : ITwitchEventSubService
    {
        ...
        public async Task<(string userId, IList<Subscription> subscriptions)> Subscribe(string username)
        {
            // Get user id
            ...
            foreach (var type in _settings.SubscriptionTypes.Where(type => currentTypes?.Any(ct => ct.Type == type) != true))
            {
                try
                {
                    var sub = await Register(type, userId);
                    if (sub != null) subs.Add(sub);
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, $"Failed to register sub { type } for user { userId }");
                }
            }
        ...
        private async Task<string> GetUserId(string username)
        {
            ...
            try
            {
                var response = await _client.GetFromJsonAsync<TwitchResponse<IdResponse>>("https://api.twitch.tv/helix/users?login=" + username);
